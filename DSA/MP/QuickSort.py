...
# CHALLENGE:
#     IMPLEMENT QUICK SORT IN THIS PROGRAM


# FUNCTION THAT SORTS THE RANGES FROM LARGEST TO SMALLEST USING QUICK SORT
def quickSort(unsortedRanges):
    unsortedRanges = list(unsortedRanges)# CONVERT THE DICTIONARY INTO LIST
    length = len(unsortedRanges)
    if length <= 1:
        return unsortedRanges
    else:
        pivot = unsortedRanges.pop()# THE PIVOT VALUE IS THE LAST ELEMENT IN THE ARRAY
        
    itemsGreater = []
    itemsLower = []
    
    for item in unsortedRanges:# THIS ITERATION CHECKS WHETHER EACH VALUES FROM THE ARRAY ARE EITHER LOWER OR HIGHER FROM THE PIVOT VALUE
        if  item[1] > pivot[1]:
            itemsGreater.append(item)# ADDS TO THE ARRAY THAT ARE HIGHER THAN PIVOT VALUE
        else:
            itemsLower.append(item)# ADDS TO THE ARRAY THAT ARE LOWER THAN PIVOT VALUE
...
def main():
    ...
    sortedRanges = quickSort(range_numberOfStudent.items())
